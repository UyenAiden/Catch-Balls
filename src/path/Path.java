 ...
 * Name(s): Uyen Hoang, Thuan La.
 * What I thought about this assignment:
 * I wish there was a demo code for shortest path.
 *
 */
...
public class Path {
    ...
    public static List<Node> shortestPath(Node v, Node end) {
        ...
        HashMap<Node, Info> S= new HashMap<Node, Info>();
        ...
        while (F.size() != 0) {
            ...
            if (f.exitsSize() == 0) return new LinkedList<Node>();
            var e= f.exits();
            for (Edge nei : e) {
                Node no= nei.other(f);
                int dis= S.get(f).dist + nei.length;
                if (!S.containsKey(no)) {
                    // add in S no, the dist from Node no to the backpointer f
                    S.put(no, new Info(dis, f));
                    F.insert(no, dis);
                } else if (S.get(no).dist > dis) {
                    S.replace(no, new Info(dis, f));
                    F.changePriority(no, dis);
                }
            }
